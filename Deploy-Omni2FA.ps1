...
# Author: Based on Omni2FA project
# License: GPLv2.1
...
function Set-NpsDllRegistry {
    ...
    # Remove any existing entries that contain Omni2FA.NPS.Plugin.dll (cleanup old installations)
    $cleanedValues = $currentValues | Where-Object { $_ -notlike "*Omni2FA.NPS.Plugin.dll*" }
...
try {
    ...
    $criticalFiles = @("Omni2FA.NPS.Plugin.dll", "Omni2FA.Adapter.dll")
    ...
    if (-not $Update) {
        ...
        $eventSources = @("Omni2FA-NPS-Plugin", "Omni2FA-NPS-Plugin.NET", "Omni2FA-Auth")
        ...
        # Step 5: Configure Omni2FA registry settings
        Write-Host "Configuring Omni2FA registry settings..." -ForegroundColor Yellow
        
        $npsWrapperPath = "HKLM:\SOFTWARE\Omni2FA"
        ...
        $npsWrapperDllPath = "$destinationPath\Omni2FA.NPS.Plugin.dll"
        if (!(Test-Path $npsWrapperDllPath)) {
            throw "Omni2FA.NPS.Plugin.dll not found at $npsWrapperDllPath"
        }
